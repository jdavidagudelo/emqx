    ...
    case emqx_json:safe_encode(Presence) of
        {ok, Payload} ->
            emqx_broker:safe_publish(
              make_msg(qos(Env), topic(connected, decode_clientid(ClientId)), Payload));
        {error, _Reason} ->
...
on_client_disconnected(_ClientInfo = #{clientid := ClientId, username := Username},
    ...
    Presence = #{clientid => decode_clientid(ClientId),
                 username => Username,
    ...
    case emqx_json:safe_encode(Presence) of
        {ok, Payload} ->
            emqx_broker:safe_publish(
              make_msg(qos(Env), topic(disconnected, decode_clientid(ClientId)), Payload));
        {error, _Reason} ->
